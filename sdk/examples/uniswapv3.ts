...
/**
 * Uniswap V3 backrunning example using Reflex SDK
 *
...
async function uniswapV3Example() {
  ...
  try {
    ...
    console.log("Executing V3 exact input swap + backrun...");
    ...
    // Example 2: Cross-tier arbitrage (0.05% vs 0.3% vs 1%)
    console.log("\n⚖️ Example 2: Cross-Tier Arbitrage");

    // Arbitrage between different fee tiers
    const crossTierResults = await Promise.all([
      // Low fee tier (0.05%)
      reflexSDK.triggerBackrun(
        WETH_USDC_V3_005,
        ethers.parseEther("1"),
        false,
        wallet.address
    ...
    crossTierResults.forEach((result, index) => {
      const feeTier = index === 0 ? "0.05%" : "0.3%";
      console.log(`✅ ${feeTier} tier result:`, {
        profit: ethers.formatEther(result.profit),
        profitToken: result.profitToken,
      });
    });
    ...
    const multiHopResult = await reflexSDK.backrunedExecute(
      multiHopExecute,
      multiHopBackrun
    );
    console.log("✅ Multi-hop backrun result:", {
      success: multiHopResult.success,
      profit: ethers.formatEther(multiHopResult.profit),
      profitToken: multiHopResult.profitToken,
    });
    ...
    const unsubscribe = reflexSDK.watchBackrunExecuted((event) => {
    ...
    // Example 5: Tick-based liquidity analysis
    console.log("\n📊 Example 5: Advanced V3 Operations");

    // Get gas estimates for different fee tiers
    const gasEstimates = await Promise.all(
      v3Pools.map(async (pool, index) => {
        try {
          const gas = await reflexSDK.estimateTriggerBackrunGas(
            pool,
            ethers.parseEther("1"),
            false,
            wallet.address
          );
          return { feeTier: feeTiers[index], gas: gas.toString() };
        } catch (error) {
          return { feeTier: feeTiers[index], gas: "Error" };
        }
      })
    );

    console.log("✅ Gas estimates by fee tier:");
    gasEstimates.forEach(({ feeTier, gas }) => {
      console.log(`   ${feeTier}: ${gas} gas`);
    });

    // Example 6: Batch V3 operations across tiers
    console.log("\n📦 Example 6: Batch V3 Operations");

    const batchV3Operations = v3Pools.map((pool, index) =>
    ...
    console.log("✅ Encoded V3 batch operations:");
    batchV3Operations.forEach((op, i) => {
      console.log(`   ${feeTiers[i]} tier:`, op.slice(0, 20) + "...");
    });
