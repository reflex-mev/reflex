...
import "@cryptoalgebra/core/interfaces/plugin/IAlgebraPluginFactory.sol";
...
interface IBasePluginV1Factory is IAlgebraPluginFactory {
    ...
    /// @notice Change default fee configuration for new pools
    /// @dev changes coefficients for sigmoids: α / (1 + e^( (β-x) / γ))
    /// alpha1 + alpha2 + baseFee (max possible fee) must be <= type(uint16).max and gammas must be > 0
    /// @param newConfig new default fee configuration. See the AdaptiveFee struct for more details about params.
    function setDefaultFeeConfiguration(AlgebraFeeConfiguration calldata newConfig) external;

    /// @notice Change farming address
    /// @dev The farming address is used in FarmingProxyPlugin
    /// @param newFarmingAddress The new farming address
    function setFarmingAddress(address newFarmingAddress) external;

    /// @notice Set the ReflexRouter address
    /// @param _reflexRouter The new ReflexRouter address
    function setReflexRouter(address _reflexRouter) external;

    /// @notice Get the current ReflexRouter address
    /// @return The current ReflexRouter address
