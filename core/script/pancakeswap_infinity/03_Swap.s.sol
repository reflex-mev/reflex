...
/// @notice Minimal interface for the PancakeSwap Universal Router execute function
...
/**
 * @title Swap
 * @notice Executes a swap on a PancakeSwap Infinity CL pool via the Universal Router.
 *
 * Run with:
 *   CL_POOL_MANAGER_ADDRESS=0x... UNIVERSAL_ROUTER_ADDRESS=0x... \
 *   TOKEN0_ADDRESS=0x... TOKEN1_ADDRESS=0x... HOOK_ADDRESS=0x... \
 *     forge script script/pancakeswap_infinity/03_Swap.s.sol \
 *     --rpc-url <RPC_URL> --private-key <PRIVATE_KEY> --broadcast
 *
 * Environment Variables (Required):
 * - CL_POOL_MANAGER_ADDRESS, UNIVERSAL_ROUTER_ADDRESS, TOKEN0_ADDRESS, TOKEN1_ADDRESS, HOOK_ADDRESS
 *
 ...
 */
contract Swap is PancakeSwapInfinityBaseScript {
    ...
    function run() external {
        ...
        bool inputIsNative = inputCurrency.isNative();

        // --- Build Infinity CL actions (processed inside Vault.lock callback) ---
        bytes memory actions = abi.encodePacked(
            uint8(Actions.CL_SWAP_EXACT_IN_SINGLE), uint8(Actions.SETTLE_ALL), uint8(Actions.TAKE_ALL)
        );

        bytes[] memory params = new bytes[](3);
        // CL_SWAP_EXACT_IN_SINGLE params
        params[0] = abi.encode(
        ...
        bytes memory commands = abi.encodePacked(uint8(0x10)); // INFI_SWAP
        ...
        if (!inputIsNative) {
            IERC20(Currency.unwrap(inputCurrency)).approve(permit2, type(uint256).max);
